...
- `create-staff-user`, `update-shop-line-settings`, `update-shop-payment-settings` — admin-only writes that RLS itself won't allow directly (`profiles`/`shops` columns are locked down), so these are the sanctioned write path; each forces the target `shop_id` to the caller's own shop server-side.
