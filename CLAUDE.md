...
**Redeeming spends the points immediately** (`0015_deduct_points_at_redeem.sql` reversed 0011 here — the customer's balance used to sit untouched until staff approved, which let them redeem again against points they'd already committed). Think of it as a purchase: tapping redeem takes the points and hands back a coupon; the coupon is the receipt. The customer still never writes the balance — all three RPCs are SECURITY DEFINER, and `create_redemption` is `service_role`-only, reached via `create-redemption` after the loyalty token proves identity. What changed is *when* the deduction happens, not *who* is trusted to do it.
- `create_redemption(customer_id, reward_id, code)` — locks the customer row, validates the reward, enforces the shop's `redeem_threshold` **and** the balance, deducts through the ledger (reason `redeem:<name>`, `staff_user_id` null since no staff member did this — it renders as "system" in the history), and inserts the `pending` coupon, all in one transaction. The balance check is authoritative, not advisory: two fast taps can't both pass it.
- `complete_redemption(p_redemption_id)` — staff/admin-approve. **Deducts nothing now**; the points already went. Keeps the `FOR UPDATE` + `status='pending'` guard so a coupon can't be marked used twice.
- `cancel_redemption(p_redemption_id)` — staff-reject, and therefore a **refund**. This makes it exactly as dangerous as the deduction: without its lock and `pending` guard, rejecting twice would hand the points back twice and mint points from nothing. Refunds get their own reason (`redeem_refund:<name>`) so they're never mistaken for an earning.

Known consequence, deliberate: an abandoned pending coupon keeps the points spent, like a voucher you bought and never used. Guards are pinned in `supabase/tests/redemptions.sql` — run `npx supabase test db` after touching any of this.

`complete_redemption`/`cancel_redemption` are granted to `authenticated` only (anon-unreachable); `create_redemption` to `service_role` only. Rewards are admin-managed directly via RLS-gated table writes (no edge function needed); redemptions are client-**read-only** (the only writers are those RPCs). Merchant UI: `RewardsPage` (admin CRUD + the redeem threshold) + `RedemptionsPage` (admin/staff pending list → Approve/Reject); customer UI: the rewards catalog + coupon view live in `customer-liff`'s `CardPage` via `customer/useRewards.ts`.
...
There is no `OPENAI_API_KEY` platform secret any more: **each shop supplies its own** (`shops.openai_api_key`, set by that shop's admin on `/settings/ai` via `update-shop-ai-settings`), and `_shared/openai.ts` takes the key as an argument rather than reading the environment. Stored plaintext, following `slip2go_api_secret`; `shops` is RLS-scoped to its own members and every select against it in this repo names columns explicitly, so **keep it that way** — a `select("*")` on `shops` in a customer-facing function would hand the key out.

Two consequences worth knowing before touching this:
- **A shop with no key must degrade, never throw.** In `line-webhook`, `createEmbedding` is the first call in `handleMessage`, whose catch only logs — so throwing there means the customer gets *silence*. It short-circuits to the canned no-answer reply instead. `ingest-file` fails the file with a "set your key first" message rather than a 500. Slip verification never touches OpenAI and keeps working regardless.
